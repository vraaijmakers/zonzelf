...
// Still raw Tailwind, unlike the rest of this page: --zon-red/amber/green are
// saturated at mid lightness, so setting 12px text in them drops this page's
// flag messages below 4.5:1 against paper. A --zon-*-deep step for each state
// would fix it everywhere at once — a design decision to raise, not a shade to
// invent in a component (CLAUDE.md, design system rules).
...
export default async function AdminBatteriesPage() {
  ...
  const { data, error } = await supabase
    .from('battery_models')
    .select('id, brand, model, sku, chemistry, voltage, capacity_ah, capacity_kwh, dod_rated, price_usd, source_url, retailer, retailer_url, image_url, scraped_at, is_published')
  ...
  return (
    <div>
      ...
      <p className="text-sm text-zon-body mb-2 max-w-2xl">
      ...
      <div className="bg-zon-blue-tint border border-zon-blue/20 rounded-lg px-4 py-3 mb-6 max-w-2xl text-sm text-zon-body">
        <p className="mb-2">
      ...
      {proposals.length > 0 && (
        <>
          <h2 className="text-xs font-semibold uppercase tracking-wide text-zon-muted mb-1">
            Proposed changes to live rows ({proposals.length})
          </h2>
          <p className="text-sm text-zon-body mb-3 max-w-2xl">
                ...
                <div key={revision.id} id={`row-${row.id}`} className="bg-zon-paper border border-zon-amber/40 rounded-lg px-4 py-3 scroll-mt-4">
                    ...
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <a
                          ...
                          className="text-xs text-zon-blue hover:underline break-all"
                        >
                  ...
                  <table className="text-xs mb-2">
                    ...
                    <tbody>
                      {changed.map(field => (
                        <tr key={field}>
                          <td className="pr-6 text-zon-body align-top">{FIELD_LABELS[field]}</td>
                          <td className="pr-6 text-zon-muted align-top line-through">
                            {formatFieldValue(field, row[field])}
                          </td>
                          <td className="font-medium text-zon-ink align-top">
      ...
      <h2 className="text-xs font-semibold uppercase tracking-wide text-zon-muted mb-3">
        Pending review ({pending.length})
      </h2>

      {pending.length === 0 && !error && (
        <p className="text-sm text-zon-muted mb-8">Nothing waiting on review right now.</p>
      )}
        ...
        {pending.map(({ row, flags }) => {
          ...
          return (
            <div key={row.id} id={`row-${row.id}`} className="bg-zon-paper border border-zon-rule rounded-lg px-4 py-3 scroll-mt-4">
                ...
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {row.sku && <span className="text-xs text-zon-muted">SKU {row.sku}</span>}
                    </div>
                    <a
                      href={row.source_url}
                      target="_blank"
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs text-zon-body mb-2">
              ...
              {dupes && dupes.length > 0 && <BatteryComparison candidate={row} others={dupes} />}
